...
foreach($object_data as $outer_list_key => $outer_list_data){
    ...
    foreach($outer_list_data as $inner_data_key => $inner_list_data){
        ...
        $a = $inner_list_data['text-input-id'];

        $tab_id = $inner_list_data['text-input-id'];
        $tab_name = $inner_list_data["text-input"]; //GETTING ONLY TABS
        $chapter_title = $inner_list_data["text-input-chapter"]; //GETTING ONLY TABS
       ...
       foreach($inner_list_data["inner-list"] as $inner_list_key => $inner_list_obj){
            ...
            if(!$question_id){
                ...
                $wpdb->update( $questions_table, array( 'question' => strip_tags($question), 'marks' => $marks), array( 'id' => $question_id  ) );
            }

           
       }
        $wpdb->update( $tabs_table, array( 'tab_name' => strip_tags($tab_name), 'chapter_title' => strip_tags($chapter_title) , 'tab_description' => strip_tags($description)), array( 'id' => $tab_id ) );
    }
}
?>
